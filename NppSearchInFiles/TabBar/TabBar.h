...
#define TCN_TABDELETE_OTHER (TCN_FIRST - 13)
#define TCN_TABDELETE_ALL (TCN_FIRST - 14)

const int marge = 8;
const int nbCtrlMax = 10;

#define CLOSE_THIS_TAB		1000
#define CLOSE_OTHER_TABS	1001
#define CLOSE_ALL_TABS		1002

class SIFTabBar : public Window
{
public:
	SIFTabBar() : Window(), _nbItem(0), _hasImgLst(false), _hFont(NULL) {};
	...
	int insertAtEnd(char *subTabName);
	...
	void setFont(char *fontName, size_t fontSize) {
		...
		_hFont = ::CreateFont( (int)fontSize, 0, 0, 0,
			                   FW_NORMAL,
				               0, 0, 0, 0,
				               0, 0, 0, 0,
					           fontName);
...
class SIFTabBarPlus : public SIFTabBar
{
public :

	SIFTabBarPlus() : SIFTabBar(), _isDragging(false), _tabBarDefaultProc(NULL), _currentHoverTabItem(-1),\
		_isCloseHover(false), _whichCloseClickDown(-1), _lmbdHit(false), m_hPopupMenu(NULL)  {};
...
protected:
	...
	static LRESULT CALLBACK TabBarPlus_Proc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
		return (((SIFTabBarPlus *)(::GetWindowLong(hwnd, GWL_USERDATA)))->runProc(hwnd, Message, wParam, lParam));
	};
	...
	int getTabIndexAt(int x, int y) {
		...
		return (int)::SendMessage(_hSelf, TCM_HITTEST, 0, (LPARAM)&hitInfo);
	};
