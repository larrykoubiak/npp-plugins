...
const char * findNextWord(const char * beginOffset, int skip) {
	while(skip > 0) {
		while(*beginOffset != ' ' && *beginOffset != '\t') {	//first skip the word
			beginOffset++;
		}
...
FILESYSTEMOBJECT * parseUNIX(const char * listItem, DIRECTORY * parent, bool isDir, bool isLink) {
	...
	if (isDir) {
		listItem = findNextWord(listItem, 5);	//go to the month
	} else {
		listItem = findNextWord(listItem, 4);	//go to the filesize
	...
	if (isLink) {	//symlink name parsing, link name syntax: 'dirname' -> 'relative link'
		...
		if (listItem[j] == '.') {	//relative path
			strcpy(fso->fullpath, parent->fso.fullpath);
			char lastChar = fso->fullpath[strlen(fso->fullpath)-1];
			if (lastChar != '/' && lastChar != '\\')
				strcat(fso->fullpath, "/");
			strcat(fso->fullpath, listItem+j);
